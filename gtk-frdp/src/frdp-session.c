...
struct _FrdpSessionPrivate
{
  ...
  gboolean redirect_clipboard;

  /* Redirection RDPDR d'un dossier local (partage "drive") */
  gchar *shared_folder_path;
                              ...
                              gpointer   user_data)
{
  ...
  if (allow_resize) {
    ...
    guint32 current_width = freerdp_settings_get_uint32 (settings, FreeRDP_DesktopWidth);
    guint32 current_height = freerdp_settings_get_uint32 (settings, FreeRDP_DesktopHeight);
    ...
    if ((current_width != request_width ||
         current_height != request_height) &&
    ...
    if (priv->scaling) {
        guint32 desktop_width = freerdp_settings_get_uint32 (settings, FreeRDP_DesktopWidth);
        guint32 desktop_height = freerdp_settings_get_uint32 (settings, FreeRDP_DesktopHeight);
...
frdp_pre_connect (freerdp *freerdp_session)
{
  ...
  static const struct {
    ...
    { NEG_DSTBLT_INDEX,             TRUE  },
    { NEG_PATBLT_INDEX,             TRUE  },
    { NEG_SCRBLT_INDEX,             TRUE  },
    { NEG_OPAQUE_RECT_INDEX,        TRUE  },
    { NEG_DRAWNINEGRID_INDEX,       FALSE },
    { NEG_MULTIDSTBLT_INDEX,        FALSE },
    { NEG_MULTIPATBLT_INDEX,        FALSE },
    { NEG_MULTISCRBLT_INDEX,        FALSE },
    { NEG_MULTIOPAQUERECT_INDEX,    TRUE  },
    { NEG_MULTI_DRAWNINEGRID_INDEX, FALSE },
    { NEG_LINETO_INDEX,             TRUE  },
    { NEG_POLYLINE_INDEX,           TRUE  },
    { NEG_MEMBLT_INDEX,             TRUE  },
    { NEG_MEM3BLT_INDEX,            FALSE },
    { NEG_MEMBLT_V2_INDEX,          TRUE  },
    { NEG_MEM3BLT_V2_INDEX,         FALSE },
    { NEG_SAVEBITMAP_INDEX,         FALSE },
    { NEG_GLYPH_INDEX_INDEX,        TRUE  },
    { NEG_FAST_INDEX_INDEX,         TRUE  },
    { NEG_FAST_GLYPH_INDEX,         FALSE },
    { NEG_POLYGON_SC_INDEX,         FALSE },
    { NEG_POLYGON_CB_INDEX,         FALSE },
    { NEG_ELLIPSE_SC_INDEX,         FALSE },
    { NEG_ELLIPSE_CB_INDEX,         FALSE },
  };
  gsize i;

  for (i = 0; i < G_N_ELEMENTS (order_support); i++) {
    BOOL value = order_support[i].value;

    freerdp_settings_set_pointer_array (settings, FreeRDP_OrderSupport, order_support[i].index, &value);
  }
...
frdp_session_set_current_keyboard_layout (FrdpSession *self) {
        ...
        for (i = 0; i < G_N_ELEMENTS (keyboard_layouts); i++) {
          if (g_strcmp0 (layout, keyboard_layouts[i].local_layout) == 0) {
            ...
            freerdp_settings_set_uint32 (settings, FreeRDP_KeyboardLayout,
                                        freerdp_keyboard_init (keyboard_layouts[i].freerdp_layout));
#endif
  ...
  if (!keyboard_layout_set)
    freerdp_settings_set_uint32 (settings, FreeRDP_KeyboardLayout, freerdp_keyboard_init (0));
#endif
}
...
frdp_session_init_freerdp (FrdpSession *self)
{
  ...
  /* freerdp_settings_set_string() copie la chaine en interne : ne pas
   * g_strdup() ici (les anciennes affectations directes settings->Xxx =
   * g_strdup(...) transferaient la propriete du buffer, set_string() en
   * cree une copie separee, un g_strdup() prealable fuirait). */
  ...
  freerdp_settings_set_bool (settings, FreeRDP_RedirectClipboard, priv->redirect_clipboard);
  freerdp_settings_set_bool (settings, FreeRDP_SupportGraphicsPipeline, TRUE);

  freerdp_client_add_dynamic_channel (settings, count, collections);

  if (priv->shared_folder_path != NULL && priv->shared_folder_path[0] != '\0') {
    CONST_QUALIFIER gchar *drive_params[] = { "drive", "partage", priv->shared_folder_path };

    freerdp_client_add_device_channel (settings, 3, drive_params);
  }
  ...
  switch (property_id)
    {
      ...
      case PROP_SHARED_FOLDER_PATH:
        g_value_set_string (value, self->priv->shared_folder_path);
        break;
      case PROP_DISPLAY:
      ...
      case PROP_REDIRECT_CLIPBOARD:
        g_value_set_boolean (value, self->priv->redirect_clipboard);
        break;
      default:
  ...
  switch (property_id)
    {
      ...
      case PROP_DOMAIN:
        ...
        break;
      case PROP_SHARED_FOLDER_PATH:
        g_free (self->priv->shared_folder_path);
        self->priv->shared_folder_path = g_value_dup_string (value);
        break;
      case PROP_DISPLAY:
      ...
      case PROP_MONITOR_LAYOUT_SUPPORTED:
        ...
        break;
      case PROP_REDIRECT_CLIPBOARD:
        self->priv->redirect_clipboard = g_value_get_boolean (value);
        break;
      default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
        break;
...
frdp_session_init (FrdpSession *self)
{
  ...
  self->priv->is_connected = FALSE;
  self->priv->redirect_clipboard = TRUE;
}
