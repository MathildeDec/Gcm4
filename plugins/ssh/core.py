...
def _render_ssh_stanza(name: str, host: str, cp: configparser.RawConfigParser, section: str) -> str:
...
def _write_ssh_config(ssh_config_path: Path, stanzas: dict[str, str]) -> None:
    ...
    block_lines.append(f"# Generated by gcm_ssh_migrate.py — {datetime.now().isoformat(timespec='seconds')}")
    block_lines.append(f"# {len(stanzas)} SSH host(s) managed here")
    if shared:
        block_lines.append("# Fichier PARTAGE — visible et utilisable par tous les utilisateurs de cette machine.")
    block_lines.append("")
    ...
    logger.info(f"_write_ssh_config | written | path={ssh_config_path} hosts={len(stanzas)} shared={shared}")
    ...
    for section in cp.sections():
        ...
        for key in GCM_KEYS_SSH_ONLY:
            if cp.has_option(section, key):
                ...
                logger.debug(f"_rewrite_gcm_conf | removed key | alias={alias} name={name} key={key}")
        ...
        logger.info(f"_rewrite_gcm_conf | patched section | alias={alias} name={name} safe_name={safe_name}")
    ...
    *,
    ...
    for section in cp.sections():
        ...
        if conn_type != GCM_TYPE_SSH:
            logger.debug(f"migrate_ssh_hosts | skip non-SSH | alias={alias} name={name} type={conn_type}")
...
def import_ssh_config(
    ...
    *,
    ...
    if dry_run:
        logger.info(f"import_ssh_config | dry-run — imported={len(result['imported'])}, nothing written")
        return result
    ...
    logger.info(f"import_ssh_config | written | path={gcm_path} imported={len(result['imported'])}")
